...
- Visible native smoke then exposed the Desktop Browser's initial `about:blank` guest being rejected before attachment. Permit that exact inert bootstrap with sandbox/preload/Node sanitization; subsequent navigation retains an HTTP(S)-only policy. Added bootstrap and prohibited-scheme regressions. Worker typecheck and 1454 selected Electron tests passed; the rebuilt signed browser is still pending at this commit.
- The visible file picker opened a disposable git workspace; file preview and Git diff showed its real contents. A synthetic tool-free response streamed, survived Rhythm tab switching, and Stop returned normal composer controls. Native PTY/file/git/camera-denial behavior also passed in the actual signed app.
