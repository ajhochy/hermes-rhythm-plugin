...
**Draft; installed/hosted cutover remains blocked.** The installed host registered `/rhythm` and loaded the diagnostic candidate but mounted no Rhythm workspace. The visible generic composer does not prove M3, hosted reads, draft handoff, or ACP behavior. The final JSX-only artifact was built and validated but not reinstalled after cleanup.
...
- Native sidebar appeared exactly once after rescans. Current module/route probes proved registration but zero workspace DOM roots; hosted interaction could not begin.
...
| #13 M9 | Final artifact installed render, full in-memory disposal/rollback; signing is a separate release gate. |
...
Detailed commands, decisions, failures, and screenshots: [finish gate](docs/ai/runs/2026-09-18-rhythm-plugin-finish-gate.md). Local fixture results do not establish deployed behavior. No merge, production takeover, OpenCode retirement, or signed release is claimed.
