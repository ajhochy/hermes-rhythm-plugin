...
def main(argv:list[str]|None=None)->int:
 ...
 for name in ("approve-design","approve-plan","check","commit","review","verify","complete","repair","force-repair","amend-scope","show","dispatch-worker","worker-status"):
  ...
  if name in {"dispatch-worker","worker-status","force-repair"}:q.add_argument("stage")
 ...
 try:
  if a.command=="create-run":out=WorkflowService(Path(a.repo)).create_run(a.package,a.scope,a.run_id,a.board,goal=a.goal)
   ...
   if a.command=="show":out=svc.show(rid)
    ...
    elif a.command=="force-repair":out=svc.force_repair(rid,actor,a.stage)
