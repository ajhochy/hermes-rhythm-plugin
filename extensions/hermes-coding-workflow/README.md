...
- `hcw worker-status <repo> <run-id> <stage>` reads back the latest worker record (`queued`, `running`, `succeeded`, or `failed`) without blocking, so the profile driving that stage can poll a long-running Claude subprocess instead of holding it synchronously.
...
Use the profile-local absolute launcher printed by the `HCW_BOOTSTRAP_V1` hook. At initial root bootstrap it gives the complete `create-run` form; in a registered stage worktree it derives the parent run only after validating the locator, authoritative state, task/profile binding, and active stage, then gives only that stage's lifecycle command(s). The public lifecycle is `create-run`, `approve-design`, `approve-plan`, `check`, `commit`, `review`, `verify`, `complete`, `repair`, `show`, `dispatch-worker`, and `worker-status`. `dispatch-worker`/`worker-status` launch and poll the external Claude Code CLI subprocess for the current `red`/`green`/`quality-review`/`complete` stage; see "Claude Code CLI worker stages" above. Both are gated the same way as every other in-lifecycle mutation: the Hermes `pre_tool_call` guard only permits them from the terminal of the profile session whose bound Kanban task matches that stage while the stage is `active`, with `<stage>` in the command required to equal that same stage — the guard never allows a different profile or a stale/inactive/unauthorized stage to dispatch or poll a worker it does not own.
