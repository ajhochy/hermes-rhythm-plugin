...
def call(argv, env, cwd):
    result = subprocess.run(argv, env=env, cwd=cwd, text=True, capture_output=True, check=True)
...
def test_installed_lifecycle_runs_real_disposable_repository(tmp_path, monkeypatch):
    ...
    base_env = {**os.environ, "HERMES_HOME":str(home), "PYTHONPATH":str(source(hermes)), "PYTHONDONTWRITEBYTECODE":"1"}
    ...
    (worktree / "tests").mkdir(); (worktree / "tests" / "__init__.py").write_text(""); (worktree / "tests" / "test_app.py").write_text("import unittest\nfrom app import value\nclass TestValue(unittest.TestCase):\n def test_value(self): self.assertEqual(value(), 2)\n")
