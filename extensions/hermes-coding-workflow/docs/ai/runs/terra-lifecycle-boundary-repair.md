...
Before any profile, payload, plugin-enable, or config mutation, the installer requires the role-worker source to name one of the documented Hermes-dispatch providers: `openai`, `anthropic`, `gemini`, `google`, `openrouter`, or `nous`. Every other provider is rejected fail-closed with `HCW_PROVIDER_BOUNDARY_UNSAFE`; direct-code routes including `openai-codex`, `copilot-acp`, `codex`, and `*-acp` are explicitly classified as such. Use `--worker-source-profile` when a preference/source profile uses a direct-code runtime but the cloned role workers use the safe Hermes route.
