...
- The service now requires the latest exact, schema-valid, identity-bound, durable create/repair-intent-bound, design/plan/dispatch-bound, successful worker record with intact hashed stdout/stderr artifacts before RED/GREEN check, GREEN commit, quality review, or completion. Redispatch after success is idempotent by default; only a durable one-shot failed RED/GREEN commit/check authorization bound to the workflow attempt, succeeded worker ID, and current worktree HEAD permits explicit `dispatch-worker --retry-succeeded`; the exact next worker receives bounded, attempt-matched gate failure details, and all terminal retries stop at three worker attempts. Repair attempts persist the exact changes-requested review as `repair-context.json`; its hash is bound into durable repair intent and a backward-compatible optional worker field (legacy dispatch hashes/records remain consumable), schema/source-validated and revalidated by the detached runner and completion gate, and its findings are rendered as count/aggregate-bounded, JSON-escaped untrusted data with explicit non-authority boundaries in RED/GREEN worker prompts. Claude file tools are auto-approved only for `./**` and run under fail-closed `dontAsk`; a real CLI smoke confirmed workspace reads succeed while `/etc/hosts` is denied. Repair attempts durably mark reused design/plan stages completed before idempotent Kanban reconciliation releases RED; a crash before reconciliation therefore leaves RED blocked and recoverable from `run.json`. Reconciliation uses the explicit core `promote --allow-triage` recovery only for a durably completed triage card, retains normal parent checks, re-reads the card, and only then completes it through supported APIs.
- The control plane also supports a narrow operator `amend-scope` transition while GREEN is active: it is actor-, revision-, and HEAD-bound; accepts additive fixed-prefix `/**` paths only; persists a crash-resumable intent and durable audit receipt; and never weakens the commit path-scope gate.
...
- The first real RED external run reached Claude's hard 12-turn cap and the resumed run reached 30, both with zero permission denials; RED now has a 60-turn bounded budget. The first GREEN implementation run likewise reached its 30-turn cap after producing source changes, so GREEN now uses the same 60-turn bounded implementation budget. The successful resumed GREEN worker then exposed that the authoritative check sandbox stripped HOME while the immutable runner requires it under `set -u`; checks now forward HOME as an explicit location variable while retaining a deny-by-default environment. A later production RED command exceeded the short check budget and returned byte streams from `TimeoutExpired`; timeouts now fail closed as `check_timeout` with no evidence, and stage skills, active-stage bootstrap guidance, and the mutation hook's exact option/type/separator parser use an explicit 600-second deterministic check budget. Model, tools, scope, and transition authority remain unchanged.
