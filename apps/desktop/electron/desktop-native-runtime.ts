...
export type DesktopRuntimeIpc = {
  handle: (channel: string, handler: (event: any, ...args: any[]) => unknown) => void
  on: (channel: string, listener: (event: any, ...args: any[]) => unknown) => void
}

export interface DesktopRuntimeWindowAdapter {
  getDialogWindow: () => unknown
  getRendererWebContents: () => any
  /** Returns a Hermes-owned Electron Session for a named auxiliary surface. */
  getPartitionSession: (partition: 'link-titles' | 'oauth') => any
  getOwnedWebContents?: () => any[]
  listOwnedWindows: () => unknown[]
  openSession: (sessionId: string, options?: unknown) => unknown
  openInstance: () => unknown
  openInTerminal: (sessionId: string, options?: unknown) => unknown
  openExternal: (url: string) => Promise<void> | void
  createOwnedPopout: (options: unknown) => unknown
  dispose: () => Promise<void> | void
}

export interface DesktopRuntimeBackendAdapter {
  ensure: (profile?: string) => Promise<unknown>
  gatewayWsUrl: (profile?: string) => Promise<unknown>
  handleApi: (request: unknown) => Promise<unknown>
  disposeOwned: () => Promise<void> | void
}

export interface DesktopLocalBackendAdapter {
  /** A host-validated local connection. Remote and SSH routes stay in the existing runtime. */
...
export interface DesktopNativeRuntimeOptions {
  ...
  register?: (channel: string, handler: (event: any, ...args: any[]) => unknown) => void
  send?: (channel: string, payload: unknown) => void
  windowAdapter?: DesktopRuntimeWindowAdapter
...
/**
 * Releases only secondary backends spawned by this Desktop runtime. Embedded
 * disposal must not invalidate the host-borrowed primary connection.
 */
export async function disposeRuntimeOwnedPool({
  clearReaper = clearInterval,
  idleReaper,
  stopAll
}: {
  clearReaper?: (timer: ReturnType<typeof setInterval>) => void
  idleReaper: ReturnType<typeof setInterval> | null
  stopAll: () => Promise<void>
}): Promise<void> {
  if (idleReaper) {
    clearReaper(idleReaper)
  }
...
/**
 * Stops only a primary process this runtime spawned. A borrowed host
 * connection has no process ownership here and must survive view disposal.
 */
export async function disposeRuntimeOwnedPrimary({
  invalidate,
  owned,
  stop,
  waitForExit
  ...
  const child = invalidate()
  stop(child)
  await waitForExit(child)
}

/** Resolves an embedded runtime's isolated storage root without affecting standalone Desktop. */
export function resolveEmbeddedHermesHome(embedded: boolean, hermesHome?: string): string | null {
  return embedded && hermesHome ? normalizeHermesHomeRoot(hermesHome) : null
}

/** Routes user-requested external links without letting embedded Desktop open a host-owned file path. */
export async function openRuntimeExternal({
  embedded,
  openEmbedded,
  openStandalone,
  rawUrl
  ...
  embedded: boolean
  openEmbedded: (url: string) => Promise<void> | void
  openStandalone: (url: unknown) => boolean
  rawUrl: unknown
}): Promise<boolean> {
  if (!embedded) {
    return openStandalone(rawUrl)
  }
  ...
  await openEmbedded(url.toString())

  return true
}

const EMBEDDED_HOST_CHANNELS = new Set([
  'hermes:embedded:metadata',
...
export function isHostOwnedEmbeddedChannel(channel: string): boolean {
  return EMBEDDED_HOST_CHANNELS.has(channel)
}

function embeddedAppFacade(hostApp: typeof electronApp, paths: DesktopNativeRuntimeOptions['paths']) {
  ...
  return new Proxy(hostApp, {
    get(target, key, receiver) {
      if (key === 'commandLine') {
        return new Proxy(target.commandLine, { get: () => noOp })
      }
      ...
      if (key === 'getPath') {
        return (name: Parameters<typeof target.getPath>[0]) => name === 'userData' && paths ? paths.userData : target.getPath(name)
      }

      if (key === 'getAppPath') {
        return () => paths?.assetRoot ?? target.getAppPath()
      }
      ...
      if (key === 'getVersion') {
        return () => typeof target.getVersion === 'function' ? target.getVersion() : 'embedded'
      }

      if (key === 'requestSingleInstanceLock') {
        return () => true
      }
...
function createEmbeddedMainWindow(contents: any) {
  return {
    webContents: contents,
    isDestroyed: () => !contents || contents.isDestroyed?.() === true,
    isFullScreen: () => false,
    isMinimized: () => false,
    isVisible: () => true,
    focus: () => contents?.focus?.(),
    // HUD mode hides the standalone primary window while its auxiliary
    // surface is visible. The embedded primary is Rhythm's child view, so it
    // must stay visible; auxiliary Hermes HUD windows still use real owned
    // BrowserWindows through the adapter.
    hide: () => undefined,
    restore: () => undefined,
    show: () => undefined
  }
...
/**
 * Initializes the real Desktop-native implementation. Importing this module is
 * inert; the standalone entry below calls it only for Hermes-owned Electron.
 * Embedded callers supply a scoped IPC registrar, a view-backed window adapter,
 * isolated paths, and an authenticated backend adapter.
 */
export function initializeDesktopNativeRuntime(options: DesktopNativeRuntimeOptions = {}) {
  ...
  const embeddedMainWindow = embedded ? createEmbeddedMainWindow(embeddedContents) : null
  const ownedWindows = new Set<any>()

  const ownedWebContents = () => {
    if (!embedded) {
      return ElectronWebContents.getAllWebContents()
    }

    return [embeddedContents, ...(runtimeWindowAdapter?.getOwnedWebContents?.() ?? [])].filter(Boolean)
  }

  const electronWebContents: any = embedded
    ? {
        fromId: (id: number) => ownedWebContents().find(contents => contents.id === id) ?? null,
        getAllWebContents: ownedWebContents,
        getFocusedWebContents: () => {
          const focused = ElectronWebContents.getFocusedWebContents()

          return ownedWebContents().includes(focused) ? focused : null
        }
      }
    : ElectronWebContents

  const ScopedBrowserWindow: any = function (windowOptions: unknown) {
    if (!runtimeWindowAdapter) {
      throw new Error('Embedded Hermes runtime requires a window adapter for a popout.')
    }

    const window: any = runtimeWindowAdapter.createOwnedPopout(windowOptions)
    ownedWindows.add(window)
    window?.once?.('closed', () => ownedWindows.delete(window))

    return window
  }

  const BrowserWindow: any = embedded
    ? Object.assign(ScopedBrowserWindow, {
        fromWebContents: (contents: unknown) => contents === embeddedContents ? embeddedMainWindow : null,
        getAllWindows: () => [...ownedWindows, ...(runtimeWindowAdapter?.listOwnedWindows() ?? [])].filter(Boolean),
        getFocusedWindow: () => embeddedMainWindow
      })
    : ElectronBrowserWindow

  const session: any = embedded && embeddedContents?.session
    ? {
        defaultSession: embeddedContents.session,
        fromPartition: (partition: string) => {
          if (!runtimeWindowAdapter) {
            throw new Error('Embedded Hermes requires a window adapter for an isolated auxiliary session.')
          }

          if (partition === 'hermes:link-titles') {
            return runtimeWindowAdapter.getPartitionSession('link-titles')
          }

          if (partition === 'persist:hermes-remote-oauth') {
            return runtimeWindowAdapter.getPartitionSession('oauth')
          }

          throw new Error(`Embedded Hermes rejected unscoped Electron partition: ${partition}`)
        }
      }
    : electronSession

  const baseIpc = options.ipc ?? {
    handle: electronIpcMain.handle.bind(electronIpcMain),
    on: electronIpcMain.on.bind(electronIpcMain)
  }

  const ipcMain: DesktopRuntimeIpc = embedded
    ? {
        handle(channel, handler) {
          if (!isHostOwnedEmbeddedChannel(channel)) {
            baseIpc.handle(channel, handler)
          }
        },
        on(channel, listener) {
          if (!isHostOwnedEmbeddedChannel(channel)) {
            baseIpc.on(channel, listener)
          }
        }
      }
    : baseIpc

  const publishConnectionDescriptor = <T>(connection: T): T => {
    options.onConnectionDescriptor?.({ type: 'upsert', connection })

    return connection
  }

  const publishConnectionRemoval = (connectionId?: string, profile?: string) => {
    options.onConnectionDescriptor?.({ type: 'remove', connectionId, profile })
  }

  const publishConnectionReset = () => {
    options.onConnectionDescriptor?.({ type: 'reset' })
  }

  const dialogParent = () => (embedded ? runtimeWindowAdapter?.getDialogWindow() : mainWindow)

const USER_DATA_OVERRIDE = embedded ? undefined : process.env.HERMES_DESKTOP_USER_DATA_DIR
...
const PRELOAD_PATH = embedded
  ? path.join(APP_ROOT, 'electron', 'preload.cjs')
...
function loadInstallStamp() {
  // Rhythm's resources belong to the host app. Embedded Desktop metadata is
  // copied into the verified artifact root, never inherited from Rhythm's
  // process.resourcesPath or a mutable source-tree build directory.
  const candidates = embedded
    ? [path.join(APP_ROOT, 'install-stamp.json')]
    : [
...
function resolveHermesHome() {
  const isolatedHome = resolveEmbeddedHermesHome(embedded, options.paths?.hermesHome)
...
const APP_ICON_PATHS = [
  ...
  ...(embedded ? [path.join(APP_ROOT, 'renderer', 'apple-touch-icon.png')] : []),
...
let embeddedMediaProtocolInstalled = false
...
function registerMediaProtocol() {
  ...
  const mediaProtocol = embedded ? embeddedContents?.session?.protocol : protocol
  mediaProtocol?.handle(MEDIA_PROTOCOL, handler)

  if (embedded) {
    embeddedMediaProtocolInstalled = true
  }
}
...
let mainWindow: any = embedded ? embeddedMainWindow : null

// `BrowserWindow.getAllWindows()` deliberately excludes the host's view-backed
// renderer facade: generic native-window loops assume the full BrowserWindow
// API. Renderer broadcasts still need to reach the embedded Desktop view.
function rendererBroadcastWindows() {
  const windows = BrowserWindow.getAllWindows()

  return embedded && mainWindow ? [mainWindow, ...windows] : windows
}
...
function resolveWebDist() {
  ...
  if (embedded) {
    return path.join(APP_ROOT, 'renderer')
  }
...
function resolveRendererIndex() {
  const candidates = embedded
    ...
    : [path.join(APP_ROOT, 'dist', 'index.html'), path.join(resolveWebDist(), 'index.html')]
...
function broadcastBatteryState(next: boolean) {
  ...
  for (const win of rendererBroadcastWindows()) {
...
function installContextMenuBridge(window: InstanceType<typeof ElectronBrowserWindow>) {
...
async function startHermes() {
  ...
  if (existingConnectionPromise) {
    return existingConnectionPromise.then(publishConnectionDescriptor)
  }
  ...
  const connectionPromise = (async () => {
    ...
    const setup = await runPrimaryBackendStartup({
      connectRemote,
      connectLocal: embedded && options.localBackend ? () => options.localBackend!.connect(primaryProfileKey()) : undefined,
  ...
  return connectionPromise.then(publishConnectionDescriptor)
}
 ...
 * the main process.
 */
function startHudCursorFeed(win: InstanceType<typeof ElectronBrowserWindow>) {
  if (process.platform !== 'linux') {
    return
  }
...
ipcMain.handle('hermes:connections:save', async (_event, payload) => {
  ...
  // Registry edits can change a remote endpoint before the next connection
  // attempt. Tell the embedded host to replace its origin entry now; the
  // descriptor stays in-process and is never exposed to the renderer bridge.
  publishConnectionDescriptor(saved)
...
ipcMain.handle('hermes:openExternal', async (_event, rawUrl) => {
  const opened = await openRuntimeExternal({
    embedded,
    openEmbedded: async url => {
      if (!runtimeWindowAdapter) {
        throw new Error('Embedded Hermes external-browser adapter is unavailable.')
      }

      await runtimeWindowAdapter.openExternal(url)
    },
    openStandalone: openExternalUrl,
    rawUrl
  })
...
return {
  async dispose() {
    // Embedded disposal is scoped to the view. The host-provided backend
    // adapter decides whether it owns a process; borrowed connections remain
    // running. The scoped IPC registrar removes handlers/listeners itself.
    closePreviewWatchers()

    if (embeddedMediaProtocolInstalled) {
      await embeddedContents?.session?.protocol?.unhandle?.(MEDIA_PROTOCOL)
      embeddedMediaProtocolInstalled = false
    }
    ...
    publishConnectionReset()
    sshBootstrapCoordinator.cancelAll()
    await Promise.allSettled([...sshConnections.keys()].map(scope => teardownSshConnection(scope || null)))
    await sshBootstrapCoordinator.forceCleanupAll()
    keepAwake.set(false)
    closeQuickEntryWindow()
    closePetOverlay()
...
export function startStandaloneDesktopNativeRuntime() {
  return initializeDesktopNativeRuntime({ mode: 'standalone' })
}
